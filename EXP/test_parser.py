...
from text_2_entity import (  # noqa: E402
    parse_input,
    parse_input_async,
    get_openai_text_response,
    open_ai_sequence_response,
    ChatMessageUser,
...
async def test_parse_input_async() -> None:
    system_content = "You are a helpful assistant that provides user information."
    user_content = "Generate information about a random user."
...
def test_get_openai_text_response() -> None:
    system_prompt = "You are a helpful assistant."
    user_prompt = "Say hello!"

    result = get_openai_text_response(
        system_prompt=system_prompt, user_prompt=user_prompt, model="gpt-4o"
    )
...
def test_open_ai_sequence_response() -> None:
    # Test with single message
    single_message = ChatMessageUser(content="Hello!", role="user")

    message_list = [single_message.model_dump()]

    result_single = open_ai_sequence_response(
        messages=message_list, model="gpt-4o"  # type: ignore
    )
    ...
    result_multiple = open_ai_sequence_response(
        messages=multiple_messages, model="gpt-4o"  # type: ignore
    )
...
async def test_get_openai_text_response_async() -> None:
    system_prompt = "You are a helpful assistant."
    user_prompt = "Say hello!"
